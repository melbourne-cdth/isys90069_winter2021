...
class MConceptSelector(ConceptSelector):
    """Handle information inside the widgets."""
    ...
    def __init__(self, topic, ontologies, value_changed=None,
        ...
        self._topic = widgets.Label(value=topic)
        self._search_input_widget = widgets.Text(value='', width='49%')
        self._search_results_widget = widgets.SelectMultiple(options=[],
                                                     width='300')

        self._container = widgets.HBox([widgets.VBox([self._topic, 
                                                     self._search_input_widget]),
                                       self._search_results_widget])
    ...
    @property
    def id(self):
        return [self._results_info[t]['@id'] for t in self.term]
...
class RadFinding(ipw.VBox):
    def __init__(self, 
                 ...
                 modifier=None):
        if not anatomy:
            anatomy = ['RADLEX', "UBERON", "FMA"]
        if not finding:
            finding = ['RADLEX', 'SNOMEDCT']
        if not modifier:
            modifier = ['RADLEX', 'SNOMEDCT']
        self.anatomy = ConceptSelector('Anatomical Location', anatomy, bioportal_api_key=apiKey)
        self.finding = ConceptSelector('Finding', finding, bioportal_api_key=apiKey)
        self.modifier = ConceptSelector('Modifier', modifier, bioportal_api_key=apiKey)
...
class RadDiagnosis(ipw.VBox):
    def __init__(self, 
                 ...
                 modifier=None):
        if not diagnosis:
            diagnosis = ['ICD10CM', "DOID"]
        if not modifier:
            modifier = ['RADLEX']

        self.diagnosis = ConceptSelector('Diagnosis', diagnosis, bioportal_api_key=apiKey)
        self.modifier = ConceptSelector('Modifier', modifier, bioportal_api_key=apiKey)
...
""")
    def __init__(self,
        ...
        if not fmodifier:
            raise VaueError("Must provide at least one finding modifier ontology")
        if not diagnosis:
            raise ValueError("Must provide at least one diagnosis ontology")
        if not dmodifier:
            raise ValueError("Must provide at least one diagnosis modifier ontology")
            
        self._report = defaultdict(list)
        self.report = ipw.HTML()
        
        self.date = ipw.DatePicker(description="Procedure Data", value=datetime.datetime.today().date())
        ...
        self.procs = ConceptSelector('Procedure', procedures, bioportal_api_key=apiKey)
        # findings
        ...
        self.findings = RadFinding(apiKey, anatomy=anatomy, finding=finding,modifier=fmodifier)
        # diagnosis
        ...
        # signature
        self.diagnosis = RadDiagnosis(apiKey, diagnosis=diagnosis, modifier=dmodifier)
        self.create = ipw.Button(description='Create Report')
        self.create.on_click(self.create_report)
        super(RadiologyReport, self).__init__(children=[ipw.VBox([ipw.HBox([self.date, self.provider]), 
                                                        ...
                                                        self.create], layout=ipw.Layout(width="50%")),
                                                        self.report], 
